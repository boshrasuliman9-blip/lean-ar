...
function show(id) {
    ...
    if (el)
        el.style.display = (id === 'pg-win' ? 'flex' : 'block');
}
function hide(id) {
    const el = document.getElementById(id);
    if (el)
...
const COLORS = { red: "#ef4444", green: "#10b981", yellow: "#f59e0b", blue: "#3b82f6" };
const DIR_TEXT = { red: "يمين", green: "يسار", yellow: "أمام", blue: "خلف" };
...
/* ── actions ── */
function showLevelRules(isCongrats = false) {
...
function showColor() {
    ...
    const cf = document.getElementById('color-fill');
    if (cf)
        cf.style.background = COLORS[c];
    sfxColor();
    const h = document.getElementById('hint');
    if (h)
        h.textContent = currentLevel <= 2 ? DIR_TEXT[c] : '';
    const sl = document.getElementById('stars-label');
    if (sl)
        sl.textContent = 'المرحلة ' + LEVELS[currentLevel].n;
    stars(levelStars);
    sfxStar();
    TIM = setTimeout(() => {
        colorOn = false;
